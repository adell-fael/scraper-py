...
const ShoppingCartDialog: React.FC = () => {
  const products = [
    {
      id: 1,
      ...
      <Dialog open={open} onClose={() => setOpen(false)} className="relative z-10">
        ...
        <div className="fixed inset-0 z-10 w-screen overflow-y-auto">
          <div className="flex min-h-full items-stretch justify-center text-center sm:items-center sm:px-6 lg:px-8">
            <DialogPanel
              transition
              className="flex w-full transform text-left text-base transition data-[closed]:scale-105 data-[closed]:opacity-0 data-[enter]:duration-300 data-[leave]:duration-200 data-[enter]:ease-out data-[leave]:ease-in sm:my-8 sm:max-w-3xl"
            >
                    ...
                    {products.map((product, productIdx) => (
                      <li key={product.id} className="flex py-8 text-sm sm:items-center">
                        <img
                          ...
                          className="h-24 flex-none rounded-lg border border-gray-200 sm:h-32"
                        />
